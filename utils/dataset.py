...
class XusRGBClsImage(Dataset):
    ...
    def __init__(self, srcpath):
        ...
        data = torch.load(self.srcpath)
        X, Y = data

        for tmp_x, tmp_y in zip(X, Y):
            self.dataset['image'].append(tmp_x)
            self.dataset['label'].append(tmp_y)
...
class XusCTSegDataset(Dataset):

    def __init__(self, srcpath, transform=None):
        self.srcpath = srcpath
        images = os.listdir(self.srcpath["image"])
        labels = os.listdir(self.srcpath["label"])
        ...
        for tmp_image, tmp_label in zip(sorted_images, sorted_labels):
            tmp_img = torch.load(os.path.join(self.srcpath["image"], tmp_image))
            tmp_lab = torch.load(os.path.join(self.srcpath["label"], tmp_label))
            self.dataset["image"].append(tmp_img)
            self.dataset["label"].append(tmp_lab)
...
class CTSegDataset(Dataset):
    ...
    def __init__(self, srcpath, transform=None):
        ...
        print(
            ...
            + "\n")
        self.img_list = [
            read_nifti_seg_file(os.path.join(imgpath, img_name))
            for img_name in tqdm(os.listdir(imgpath))
        ]
        print(
            "<<<<<<<<<<<<<<<<<<<<Waiting For Labels Loading<<<<<<<<<<<<<<<<<<<<"
            + "\n")
        self.label_list = [
            get_nifti_seg_label(os.path.join(labelpath, label_name))
            for label_name in tqdm(os.listdir(labelpath))
        ]
        print("<<<<<<<<<<<<<<<<<<<<Data Loading Complete<<<<<<<<<<<<<<<<<<<<" +
              "\n")
...
def load_random_split_data(srcpath):
    ...
    if task == "classification" and data_type == "CT":
        dataset = XusCTClsDataset(srcpath)
        trainloader, testloader, num_examples = random_split_data_loader(
            dataset)
    ...
    if task == "classification" and data_type == "RGB":
        ...
        dataset = XusRGBClsImage(srcpath)
        trainloader, testloader, num_examples = random_split_data_loader(
            dataset)
    ...
    if task == "segmentation" and data_type == "CT":
        dataset = XusCTSegDataset(srcpath,
...
def load_data_from_path(train_path, test_path):
    ...
    if task == "classification" and data_type == "CT":
        train_dataset = XusCTClsDataset(train_path)
        test_dataset = XusCTClsDataset(test_path)
    ...
    if task == "classification" and data_type == "RGB":
        ...
        train_dataset = XusRGBClsImage(train_path)
        test_dataset = XusRGBClsImage(test_path)
        trainloader, testloader, num_examples = data_loader(
            train_dataset, test_dataset)
    ...
    if task == "segmentation" and data_type == "CT":
        train_dataset = XusCTSegDataset(train_path,
                                        transform=transforms.Compose([
                                            RandomRotFlip(),
                                            RandomCrop(
        ...
        test_dataset = XusCTSegDataset(test_path,
                                       transform=transforms.Compose([
                                           RandomRotFlip(),
                                           RandomCrop((img_size[0], img_size[1],
                                                       img_size[2])),
